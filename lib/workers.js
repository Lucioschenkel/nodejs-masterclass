...
workers.validateCheckData = function(originalCheckData) {
  ...
  originalCheckData.userPhone = typeof(originalCheckData.userPhone) === 'string' && originalCheckData.userPhone.trim().length > 8 ? originalCheckData.userPhone : false;
  ...
  originalCheckData.timeoutSeconds = typeof(originalCheckData.timeoutSeconds) === 'number' && originalCheckData.timeoutSeconds <= 5 && originalCheckData.timeoutSeconds >= 2 ? originalCheckData.timeoutSeconds : false;

  // Set the keys that may not be set if the workers have never seen this check before
  originalCheckData.state = typeof(originalCheckData.state) === 'string' && ['up', 'down'].indexOf(originalCheckData.state) > -1 ? originalCheckData.state : 'down';
  originalCheckData.lastChecked = typeof(originalCheckData.lastChecked) === 'number' && originalCheckData.lastChecked > 0 ? originalCheckData.lastChecked : false;

  // If the all the checks pass, pass the data along to the next step in the process
  if (
...
// Perform the check, send the originalCheckData and the outcome of the check procedure to the next process
...
// Process the check outcome, update the check data if needed, trigger an alert if needed
// Special logic for accomodating a check that has never been tested before (dont s)
          ...
          if (!err) {
            ...
            _logs.truncate(logId, function(err) {
              if (!err) {
                ...
                debug('Erro truncating log files', err);
              }
            });
          } else {
...
// Time to execute the log rotation process once a day
...
workers.init = function() {
  ...
  // Call the loop so the checks continue to execute later on
  workers.loop();
