...
cli.responders.help = function () {
  const commands = {
    ...
    'list checks --up | list checks --down': 'Show a list of all the active checks in the system, including the system. The --up and --down flags are optional',
    'more check info --{checkId}': 'Show details of a specific check',
    'list logs': 'Show a list of all the log files available to be read (compressed only)',
    'more log info --{fileName}': 'Show details of a specified log file'
  };

  // Show a header for the help page that is as wide as the string
  cli.horizontalLine();
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(function(checkId) {
        _data.read('checks', checkId, function(err, checkData) {
          ...
          // If the user has specified any state, or has not specified nay state, include the check
    ...
    _logs.decompress(logFileName, function(err, stringData) {
      if (!err && stringData) {
        ...
        arr.forEach(function(jsonString) {
          let logData = helpers.parseJsonToObject(jsonString);
          if (logData && JSON.stringify(logData) !== '{}') {
            console.dir(logData, { 'colors': true });
            cli.verticalSpace();
          }
        });
      }
...
cli.processInput = function (str) {
  ...
  if (str) {
    // Codify the unique strings that identify the unique questions allowed to be answered
    const uniqueInputs = [
      'man',
      'help',
      'exit',
